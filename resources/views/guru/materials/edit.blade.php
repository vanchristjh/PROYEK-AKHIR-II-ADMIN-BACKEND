...
@section('content')
    <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 mb-6 text-white relative overflow-hidden animate-fade-in">
        ...
        <div class="relative z-10">
            ...
            <p class="text-blue-100">Perbarui materi pembelajaran untuk siswa.</p>
        </div>
    </div>

    <div class="mb-6">
        <a href="{{ route('guru.materials.show', $material) }}" class="inline-flex items-center text-blue-600 hover:text-blue-800 transition-colors">
        ...
        <div class="p-6">
            <form action="{{ route('guru.materials.update', $material) }}" method="POST" enctype="multipart/form-data" class="animate-fade-in" id="materialForm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="form-group mb-5 md:col-span-2">
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            <textarea name="description" id="description" rows="6" 
                                class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50 transition-shadow duration-300" required>{{ old('description', $material->description) }}</textarea>
                        </div>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                                ...
                                @foreach($classrooms as $classroom)
                                    <div class="flex items-center">
                                        <input type="checkbox" id="class_{{ $classroom->id }}" name="classroom_id[]" value="{{ $classroom->id }}" 
                                            ...
                                            {{ (old('classroom_id') && in_array($classroom->id, old('classroom_id'))) || 
                    ...
                    <div class="form-group mb-5 md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Lampiran Materi Saat Ini</label>
                        @if($material->file_path)
                            <div class="mt-1 bg-gray-50 rounded-lg border border-gray-200 p-4 mb-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10 rounded-lg flex items-center justify-center">
                                        <i class="fas {{ $material->file_icon }} {{ $material->file_color }} text-xl"></i>
                                    </div>
                                    <div class="ml-3 flex-1">
                                        <div class="text-sm font-medium text-gray-900">{{ basename($material->file_path) }}</div>
                                        <div class="text-xs text-gray-500">{{ strtoupper($material->file_extension) }} File</div>
                                    </div>
                                    <div>
                                        <a href="{{ asset('storage/' . $material->file_path) }}" target="_blank" class="text-blue-600 hover:text-blue-800">
                        ...
                        @else
                            <div class="mt-1 bg-gray-50 rounded-lg border border-gray-200 p-4 mb-4">
                                <p class="text-gray-500 text-sm">Tidak ada file lampiran</p>
                            </div>
                        @endif
                        ...
                        <div class="mt-1 relative">
                            <div class="flex items-center">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Maksimal 20MB. Format: pdf, doc, docx, xls, xlsx, ppt, pptx, jpg, png, mp4, zip. Biarkan kosong untuk tetap menggunakan file yang ada.</p>
                                </div>
                            </div>
                        </div>
                        @error('attachment')
                ...
                <div class="border-t border-gray-200 mt-8 pt-5">
                    <div class="flex justify-end">
                        <a href="{{ route('guru.materials.show', $material) }}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors duration-300">
